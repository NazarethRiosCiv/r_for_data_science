...
# whether or not your data meets your expectations. To do data cleaning,
# you'll need to deploy all the tools of EDA: visualization, transformation,
# and modeling.
...
# investigation. The key to asking quality questions is to generate a
# large quantity of questions. 

# Two types of questions will always be useful for making discoveries within
# your data:
...
# An observation (or data point) is a set of measurements made under similar 
...
# several values, each associated with a different variable. 

# Tabular data is a set of values, each associated with a variable and 
# an observation. Tabular data is tidy if each value is placed in its
# own cell, each variable in its own column, and each observation in 
# its own row. 

### Variation #################################################################

# Variation is the tendency of the values of a variable to change from
# measurement to measurement. Each of your measurements will include a
# small amount of error that varies from measurement to measurement. 

# Categorical variables can also vary if you measure across different
# subjects, or different times. Every variable has its own pattern of
# variation, and the best way to understand that pattern is to visualize
# the distribution of the variable's values.

## Visualizing Distributions **************************************************

# How you visualize the distribution of a variable will depend on whether
# the variable is categorical or continuous. 
...
# character vectors. To examine the distribution of a categorical
...
# ordered values. To examine the distribution of a coninuous variable,
# use a histogram:
ggplot(data = diamonds) + 
...
# If you wish to overlay multiple histograms in the same plot, it's
# recommended to use geom_freqpoly() instead, which performs the 
# same calculation as geom_histogram() but uses lines:
smaller <- diamonds %>% 
  filter(carat < 3)
...
# In general, clusters of similar values suggest that subgroups exits
# in your data. To understand the subgroups:
...
# fit the pattern. Sometimes they're data entry errors; other times they 
# suggest important new science. 

# When you have a lot of data, outliers are sometimes hard to see in a 
# histogram:
ggplot(diamonds) +
...
# they're there, it's reasonable to replace them with missing values. 
# However, if they have a substantial effect on your results, you shouldn't
# drop them without justification. You'll need to figure out what caused
# them and disclose that you removed them in your write-up.
...
# Replace the outliers with missing values (recommended)
diamonds2 <- diamonds %>% 
...
# values different from observations with recorded values:
...
# The above plot isn't great because there are many more non-cancelled flights
# than cancelled flights. There are better techniques for this situation.

### Co-variation ##############################################################

# If variation describes the behavior within a variable, co-variation 
# describes the behavior between variables. Co-variation is the tendency
# for the values of two or more variables to vary together in a related way. 
# The best way to spot co-variation is to visualize the relationship between
...
# The default appearance of geom_freqpoly() is not that useful for that sort
# of comparison because the height is the count, which means if one of the 
# groups is much smaller than the others, it's hard to see the difference
# in shape.
...
# the count standardized so that the area under each frequency polygon is 1.
ggplot(
  data = diamonds,
...
# Another alternative to show relationship between a continuous and 
# categorical variable is the box-plot. The cut is an ordered factor:
# fair is worse than good, which is worse than very good. It makes sense
# to display the boxplots in this order:
ggplot(data = diamonds, mapping = aes(x = cut, y = price)) + 
  geom_boxplot()

# Many categorical variables don't have such an ordinal nature, so you 
# may want to reorder (sort) them to make a more intuitive display:
...
# If you have long variable names, the box-plot will work better if you 
...
# To visualize co-variation between categorical variables, you'll need to 
# count the number of observations for each combination. 
ggplot(data = diamonds) + 
  geom_count(mapping = aes(x = cut, y = color))

# Co-variation will appear as a strong correlation between specific x values
# and specific y values.
...
# order to more clearly reveal interesting patterns. For larger plot,
# try the "d3heatmap" or "heatmaply" packages.

## Two Continuous Variables ***************************************************

# A great way to visualize the covariation between two  continuous variables
# is to make a scatterplot where you can see the the covariation as a pattern
# in the points.

# For example, you can see the exponential relationship between the carat size
# and price of the diamond:
ggplot(data = diamonds) + 
  geom_point(mapping = aes(x = carat, y = price))

# Scatterplots become less useful as the size of your dataset grows, because
# points begin to overplot and pile up into areas of uniform black. One way 
# to fix this problem is to add transparency to the points:
ggplot(data = diamonds) + 
  geom_point(
...
# solution is to use bin, namely in two dimensions. geom_bind2d() and 
# geom_hex() divide the coordinate plane into 2D bins and then use a fill
...
# categorical variable. This way you can use the techniques discussed 
# regarding the covariation of a categorical and continuous variable. 

# You could bin carat and then for each group display a boxplot. Here, 
# cut_width() divides x into bins of specified width. 
...
# relationship (a special case), then you can use the value of one variable
# to control the value of the other. 

# Models are a tool for extracting patterns out of data. It's possible to use
# a model to remove the strong relationship between two variables so we can 
# explore the subtleties that remain. 

# The following code fits a model that predicts price and carat and then 
# computes the residuals (the difference between the predicted values and
# the actual value). The residuals give us a view of the price of the diamond
# once the effect of carat is removed:
library(modelr)
...
# Once you've removed teh strong relationship between carat and price, you 
# can see what you expect in the relationship betweet cut and price; relative
